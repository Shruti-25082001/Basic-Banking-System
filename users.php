    ...
    if (($amount)<0)
   {
    ...
    //For 0 (zero) value
         ...
         echo "</script>";
                ...
                $sql = "UPDATE `customer` set `Amount`=$newbalance where `Account No.`=$from";
                mysqli_query($conn,$sql);
             
                $newbalance = $sql2['Amount'] + $amount;
                $sql = "UPDATE `customer` set `Amount`=$newbalance where `Account No.`=$to";
                mysqli_query($conn,$sql);
...
<div class="main_div">
 ...
 <div class="navbar navbar-expand-md">
  ...
  <a href="#" class="navbar-brand font-weight-bold text-white text-center"><h2>SAMRIDDHI BANK</h2></a>
 ...
 <img src = "07.jpg " class="center" width = 100% height = 70%>;

 </div>
 

 <class class="welcome">
        <h2 class="text-center pt-4" style="color : black;"> <strong>Money Transfer</strong>   </h2>
</class>
